...
//        String connString = "https://c9p5mwld41:45zeygn9hy@kafka-course-2322630105.eu-west-1.bonsaisearch.net:443";
    ...
    public static void main(String[] args) throws IOException {
        ...
        // we need to create an index on OpenSearch does not exist already
        try (openSearchClient; consumer) {
            ...
            while (true){
                ...
                log.info("Recived "+  recordCount + " record(s)");

                BulkRequest bulkRequest = new BulkRequest();

                for (ConsumerRecord<String, String> record: records){
                    // send the record into OpenSearch
                    // strategy 1 to make idempotent
                    // define and ID using Kafka Record coordinates
                    // String id =  record.topic() + "_" + record.partition() + "_" + record.offset();

                    try {
                        // strategy 2 to make idempotent
                ...
                if(bulkRequest.numberOfActions() > 0){
                    ...
                    consumer.commitSync(); // commit only after successfully processed the entire batch
